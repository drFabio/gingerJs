...
function Ginger(){
	...
	this._config={};
	/**
	 * The configuration file it is used for fallBack for example if a component is asked and isn't explicit disabled
	 * @type {Object}
	 */
	this._defaultConfig={};
	/**
...
Ginger.prototype.up = function(cb) {
	this._defaultConfig=require(__dirname+'/config/default.js');
	this._loadAbstracts();
	var self=this;
...
Ginger.prototype.getConfigValue = function(name) {
	...
	else if(this._defaultConfig[name]){
...
Ginger.prototype.setConfig = function(config,merge) {
	...
	if(_.isEmpty(config)){
		...
		if(merge){
			this._config = _.merge({},this._defaultConfig, this._config);
		}
	}
};
...
Ginger.prototype.isComponentSet=function(name){
	...
	if(this._defaultConfig.components && !!this._defaultConfig.components[name]){
		return true;
	}
	return false;
}
...
Ginger.prototype._createBootstrapper = function(name,params) {
	...
	var BootStraperClass;
		if(!params){
			...
			if(this._config.bootstrappers && !!this._config.bootstrappers[name]){
					params=this._config.bootstrappers[name];
			}
			else if(this._defaultConfig.bootstrappers && !!this._defaultConfig.bootstrappers[name]){
				params=this._defaultConfig.bootstrappers[name];
			}
			else{
				params={};
			}
		...
		path=this._config.bootstrapDir+name+'.js';
		if(fs.existsSync(path)){
			BootStraperClass=require(path);

		}
		//The file does'nt exists and the dirs are different
		else if(this._config.bootstrapDir!=this._defaultConfig.bootstrapDir){
			path=this._defaultConfig.bootstrapDir+name+'.js';
			if(fs.existsSync(path)){
				BootStraperClass=require(path);
			}
			else{
...
Ginger.prototype._createComponent = function(name,params) {
	...
	var ComponentsClass;
		if(!params){
			...
			else if(this._defaultConfig.components && !!this._defaultConfig.components[name]){
				params=this._defaultConfig.components[name];
			}
			else{
				params={};
			}
		...
		path=this._config.componentDir+name+'.js';
		if(fs.existsSync(path)){
			ComponentsClass=require(path);

		}
		//The file does'nt exists and the dirs are different
		else if(this._config.componentDir!=this._defaultConfig.componentDir){
			path=this._defaultConfig.componentDir+name+'.js';
			if(fs.existsSync(path)){
				ComponentsClass=require(path);
			}
			else{
...
Ginger.prototype.getGatewayClass = function(name,params,getDefault,path) {
	...
	if(!params){
		...
		else if(this._defaultConfig.gateways && !!this._defaultConfig.gateways[name]){
				params=this._defaultConfig.gateways[name];
		}
	}
	if(!path){
		if(getDefault || this._config.gatewayDir!=this._defaultConfig.gatewayDir){
			path=this._defaultConfig.gatewayDir+name+'.js';
		}
